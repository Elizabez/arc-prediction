...
function BadgeCard({ quiz, completed, unlocked, onClick }: {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: '11px', fontWeight: 700, color: '#e2e8f0', marginBottom: '4px', lineHeight: 1.3 }}>
        {quiz.title}
      </div>
      <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '8px' }}>{quiz.difficulty}</div>
      <div style={{ fontSize: '11px', fontWeight: 600 }}>
        {completed
          ? <span style={{ color: '#3b82f6' }}>✓ Earned</span>
          : unlocked
...
function QuizPlayer({ quiz, onPass, onFail, onBack }: {
  quiz: QuizData
  onPass: (answerIndex: number) => void
  onFail: () => void
  onBack: () => void
}) {
  ...
  const isCorrect = revealed && selected === quiz.correctIndex
  ...
  return (
    <div className="quiz-player">
      ...
      <div className="quiz-header">
        ...
        <h2>{quiz.title}</h2>
        <span className="step-indicator">{quiz.difficulty}</span>
      </div>
      <div className="question-box">
        ...
        {!revealed
          ...
          : (
            <div>
              <div className={`feedback-msg ${isCorrect ? 'correct' : 'wrong'}`} style={{ marginBottom: '12px', padding: '10px', borderRadius: '8px', textAlign: 'center', fontWeight: 600 }}>
                {isCorrect ? '✓ Correct!' : `✗ Wrong! Answer: ${quiz.options[quiz.correctIndex]}`}
              </div>
              <button className="confirm-btn" onClick={handleNext}>
                {isCorrect ? 'Mint Badge 🎉' : 'Try Again'}
              </button>
            </div>
          )
        }
...
function MintBadge({ quiz, answerIndex, onDone }: {
  quiz: QuizData
  answerIndex: number
  onDone: () => void
}) {
  ...
  function handleMint() {
    writeContract({
      ...
      args: [BigInt(quiz.id), answerIndex],
  ...
  return (
    <div className="mint-screen">
      ...
      <h2 style={{ textAlign: 'center' }}>Correct Answer!</h2>
      ...
      {isSuccess && (
        <div style={{ textAlign: 'center', marginTop: '16px' }}>
          ...
          <button className="mint-btn" onClick={onDone} style={{ marginTop: '16px', width: '100%' }}>
            ← Back to Quizzes
          </button>
        </div>
      )}
...
export default function QuizPage() {
  ...
  useEffect(() => {
    const timer = setInterval(() => setUnlockedCount(getArcUnlockedCount()), 60_000)
  ...
  const progress = progressData ? [...progressData] : Array(30).fill(false)
  const completedCount = progress.filter(Boolean).length

  function handlePass(answerIndex: number) {
    ...
    <div className="quiz-page">
        ...
        <>
          <div className="quiz-page-header">
            ...
            <p className="subtitle">
              {isConnected
                ? `${completedCount}/${unlockedCount} completed · Badges: ${myBadgeCount?.toString() ?? '0'} · ${30 - unlockedCount} coming soon`
          ...
          {unlockedCount < 30 && (
            <div>
              <div style={{ fontSize: '12px', fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '14px' }}>
                🔒 Coming Soon ({30 - unlockedCount} quizzes)
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '14px' }}>
      ...
      {state === 'playing' && activeQuiz && (
        <QuizPlayer
          quiz={activeQuiz}
          onPass={handlePass}
          onFail={() => { setActiveQuiz(activeQuiz); setState('playing') }}
          onBack={() => setState('list')}
        />
      )}

      {state === 'result' && activeQuiz && correctAnswer !== null && (
        <MintBadge quiz={activeQuiz} answerIndex={correctAnswer} onDone={handleDone} />
      )}
    </div>
  )
}
