...
const BADGE_TIERS = [
  ...
  { maxId: 99, label: 'Diamond', color: '#22c55e', bg: '#22c55e14', border: '#22c55e60' },
]
...
const USER_LEVELS = [
  ...
  { min: 15, max: 20,       label: 'Master',   color: '#22c55e', icon: '💎' },
...
function MintBadge({ quiz, answers, onDone }: {
  ...
  const tier = getBadgeTier(quiz.id)
  const refUrl = `${APP_URL}?ref=${address?.slice(2, 10) ?? 'rh'}`
  ...
  useEffect(() => {
    ...
    if (!code) return
    setRefTracked(true)
    resolveRefCode(code, robinhoodTestnet.rpcUrls.default.http[0], RH_QUIZ_CONTRACT).then(referrer => {
  ...
  function handleMint() {
    writeContract({
      address: RH_QUIZ_CONTRACT, abi: ROBINHOOD_QUIZ_ABI,
  ...
  function shareOnX() {
    const text = `Just minted "${quiz.emoji} ${quiz.title}" Soulbound NFT on #RobinhoodChain Testnet! 🔴\n\nEarn yours free on Quiz:`
    ...
    <div style={{ maxWidth: '480px', margin: '0 auto', padding: '60px 24px 80px', textAlign: 'center' }}>
      ...
      {!isSuccess && (
        <button onClick={handleMint} disabled={isPending || isConfirming} style={{
          ...
          {isPending ? 'Confirm in wallet…' : isConfirming ? '⏳ Minting…' : '🔴 Mint Robinhood Badge'}
        </button>
      )}
      ...
      {isSuccess && (
        <div style={{ marginBottom: '12px' }}>
          ...
          <div style={{ color: ACC, fontWeight: 700, fontSize: '16px', marginBottom: '4px' }}>Badge minted on Robinhood Chain!</div>
          {hash && (
            <a href={`https://explorer.testnet.chain.robinhood.com/tx/${hash}`} target="_blank" rel="noreferrer"
              style={{ color: '#22c55e', fontSize: '13px', display: 'block', marginBottom: '20px', textDecoration: 'none' }}>
              View on Explorer →
            </a>
          )}
...
// ── Main Dashboard ─────────────────────────────────────────────────
export default function RobinhoodDashboardPage() {
  const { address, isConnected } = useAccount()
  const [quizState, setQuizState] = useState<'dashboard' | 'playing' | 'result'>('dashboard')
  const [activeQuiz, setActiveQuiz] = useState<QuizData | null>(null)
  const [correctAnswers, setCorrectAnswers] = useState<number[] | null>(null)
  const [copied, setCopied] = useState(false)
  const [unlockedCount] = useState(getRobinhoodUnlockedCount())

  const refUrl = `${APP_URL}?ref=${address?.slice(2, 10) ?? 'rh'}`

  const { data: progressData, refetch } = useReadContract({
    address: RH_QUIZ_CONTRACT, abi: ROBINHOOD_QUIZ_ABI, functionName: 'getUserProgress',
    args: address ? [address] : undefined,
    chainId: robinhoodTestnet.id, query: { enabled: !!address },
  })
  const { data: totalMinted } = useReadContract({
    address: RH_QUIZ_CONTRACT, abi: ROBINHOOD_QUIZ_ABI, functionName: 'totalMinted',
    chainId: robinhoodTestnet.id,
  })
  const { data: myBadges } = useReadContract({
    address: RH_QUIZ_CONTRACT, abi: ROBINHOOD_QUIZ_ABI, functionName: 'balanceOf',
    args: address ? [address] : undefined,
    chainId: robinhoodTestnet.id, query: { enabled: !!address },
  })
  const { data: myRefCount } = useReadContract({
    ...
    args: address ? [address] : undefined,
    chainId: robinhoodTestnet.id, query: { enabled: !!address && !!REFERRAL_CONTRACT },
  })

  const progressArr = progressData ? [...progressData] : Array(ROBINHOOD_QUIZZES.length).fill(false)
  ...
  function shareOnX() {
    const text = `Just earned Soulbound NFT badges learning #RobinhoodChain! 🔴\nJoin me on Quiz — learn & earn for free:`
    window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(refUrl)}`, '_blank')
  }
  ...
  return (
    <div style={{ maxWidth: '1140px', margin: '0 auto', padding: '32px 24px 80px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
        ...
        {address && (
          <span style={{ fontSize: '12px', color: '#475569', fontFamily: 'monospace', marginLeft: 'auto' }}>
            {address.slice(0, 6)}...{address.slice(-4)} · Robinhood Testnet
          </span>
        )}
        {!isConnected && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '24px' }}>
        {[
          { icon: '🎓', value: `${completedCount}`, sub: `/ ${ROBINHOOD_QUIZZES.length}`, label: 'Completed', color: '#22c55e' },
          ...
          { icon: '🔓', value: `${unlockedCount}`, sub: `/ ${ROBINHOOD_QUIZZES.length}`, label: 'Unlocked', color: '#38bdf8' },
        ].map(s => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ background: '#0d1424', border: `1px solid ${ACC}33`, borderRadius: '16px', padding: '22px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              ...
              <span style={{ fontSize: '12px', color: '#64748b', fontFamily: 'monospace' }}>{completedCount}/{nextLevel ? nextLevel.min : ROBINHOOD_QUIZZES.length}</span>
            </div>
          ...
          <div>
            <h2 style={{ fontSize: '13px', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 12px', paddingBottom: '10px', borderBottom: `1px solid ${ACC}33` }}>
              Robinhood Badges · {completedCount}/{ROBINHOOD_QUIZZES.length} earned
            </h2>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
              {BADGE_TIERS.map(t => (
                <div key={t.label} style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '10px', fontWeight: 700, color: t.color }}>
                  ...
                  {t.label} ({t.maxId <= 5 ? '1–5' : t.maxId <= 10 ? '6–10' : t.maxId <= 15 ? '11–15' : '16'})
                </div>
              ))}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px, 1fr))', gap: '10px' }}>
              {ROBINHOOD_QUIZZES.map((quiz, i) => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: '#0d1424', border: `1px solid ${ACC}33`, borderRadius: '16px', padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
              ...
              <div>
                <div style={{ fontSize: '15px', fontWeight: 800, color: ACC }}>Robinhood Chain</div>
                <div style={{ fontSize: '11px', color: '#64748b' }}>Testnet · Chain ID 46630</div>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <SocialLink icon={<XIcon />}    label="@RobinhoodApp"       url="https://x.com/RobinhoodApp"                              color="#fff" />
              <SocialLink icon={<GlobeIcon />} label="robinhood.com/chain" url="https://robinhood.com/us/en/support/robinhood-chain/"    color={ACC} />
              <SocialLink icon={<GlobeIcon />} label="Chain Explorer"      url="https://explorer.testnet.chain.robinhood.com"            color="#64748b" />
              <SocialLink icon={<GlobeIcon />} label="Sepolia Faucet"      url="https://sepoliafaucet.com"                               color="#10b981" />
            </div>
          </div>
          ...
          <div style={{ background: '#0d1424', border: `1px solid ${ACC}22`, borderRadius: '16px', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', borderBottom: `1px solid ${ACC}22`, display: 'flex', alignItems: 'center', gap: '8px' }}>
              <XIcon />
              <span style={{ fontSize: '13px', fontWeight: 700, color: '#e2e8f0' }}>Latest from @RobinhoodApp</span>
              <a href="https://x.com/RobinhoodApp" target="_blank" rel="noreferrer" style={{ marginLeft: 'auto', fontSize: '11px', color: ACC, textDecoration: 'none' }}>View all →</a>
            </div>
            <TwitterFeed handle="RobinhoodApp" />
          </div>
