          ...
          : (
            <div>
              <div className={`feedback-msg ${isCorrect ? 'correct' : 'wrong'}`}
                style={{ marginBottom: '12px', padding: '10px', borderRadius: '8px', textAlign: 'center', fontWeight: 600 }}>
    ...
    <div className="mint-screen">
      ...
      {isSuccess && (
        <div style={{ textAlign: 'center', marginTop: '16px' }}>
          ...
          <button className="mint-btn tempo" onClick={onDone} style={{ marginTop: '16px', width: '100%' }}>
            ← Back to Quizzes
          </button>
        </div>
      )}
...
export default function TempoQuizPage() {
  ...
  useEffect(() => {
    const timer = setInterval(() => setUnlockedCount(getTempoUnlockedCount()), 60_000)
  ...
  const progress = progressData ? [...progressData] : Array(50).fill(false)
        ...
        <>
          <div className="quiz-page-header">
            ...
            <p className="subtitle">
              {address
                ? `${completedCount}/${unlockedCount} completed · Badges: ${myBadges?.toString() ?? '0'} · ${50 - unlockedCount} coming soon`
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '14px' }}>
              {TEMPO_QUIZZES.slice(0, unlockedCount).map(quiz => (
                <BadgeCard
                  ...
                  unlocked={true}
                  onClick={() => { setActiveQuiz(quiz); setState('playing'); setCorrectAnswer(null) }}
                />
              ))}
            </div>
          </div>
          ...
          {unlockedCount < 50 && (
            <div>
              <div style={{ fontSize: '12px', fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '14px' }}>
                🔒 Coming Soon ({50 - unlockedCount} quizzes)
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '14px' }}>
