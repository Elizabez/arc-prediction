  ...
  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', padding: '60px 24px 80px', textAlign: 'center' }}>
      ...
      <div style={{
        ...
        background: `linear-gradient(145deg, #0a0f1a 0%, ${tier.color}10 50%, #3b82f60a 100%)`,
        ...
        <div style={{ position: 'absolute', top: '-50px', left: '-50px', width: '160px', height: '160px', borderRadius: '50%', background: '#3b82f606', border: '1px solid #3b82f612' }} />
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{
            ...
            background: '#3b82f615', border: '1px solid #3b82f628',
...
function NextUnlockCountdown({ unlockedCount }: { unlockedCount: number }) {
  ...
  const nextQuiz = ARC_QUIZZES[unlockedCount]
  const ACC = '#3b82f6'

  useEffect(() => {
    if (!nextQuiz) return
    const unlockDate = getArcUnlockDate(nextQuiz.id)
    function update() {
      ...
      <div className="dash-2col">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div>
            ...
            {unlockedCount < ARC_QUIZZES.length && (
              ...
              {ARC_QUIZZES.map((quiz, i) => {
                ...
                return (
                  <div key={quiz.id}
                    ...
                    style={{
                      background: done ? tier.bg : clickable ? `linear-gradient(135deg, ${tier.bg}, #0d1424 70%)` : '#0d1424',
